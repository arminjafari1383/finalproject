...
export default function Referrals() {
  ...
  useEffect(() => {
    ...
    if (tg) {
      ...
      if (tg.initDataUnsafe?.user) {
        ...
        console.log("✅ [DEBUG] Telegram ID:", tgId);
      ...
      console.log("⚠️ [DEBUG] Telegram WebApp NOT found - browser mode");
  ...
  useEffect(() => {
    ...
    console.log("💾 [DEBUG] Wallet address saved:", address);
  }, [address]);
  ...
  async function fetchTestData() {
    ...
    try {
      ...
      setError(
        `Failed to load test data: ${e?.response?.data?.error || e?.message || "Unknown error"}`
      );
    } finally {
      setLoading(false);
    }
  ...
  function renderLevelTable(level, data) {
    ...
    return (
      <div className="level-table">
        ...
        {users.length > 10 && <div className="show-more">+ {users.length - 10} more users</div>}
      </div>
    );
  }
